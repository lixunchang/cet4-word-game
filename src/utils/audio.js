    ...
    class AudioManager {
      constructor() {
        ...
        this.letterAudios = new Map(); // 存储字母音频
        this.onlineAudios = new Map(); // 存储在线音乐
        this.playingLetters = new Set(); // 正在播放的字母
        this.currentOnlineAudio = null; // 当前播放的在线音乐
        
        this.letterVolume = 0.7;
        this.onlineVolume = 0.7;
        this.isLooping = false;
        
        // 初始化字母音频URL（这里使用模拟URL，实际使用时需要替换为真实URL）
        // this.letterUrls = {};
        // const letters = 'abcdefghijklmnopqrstuvwxyz'.split('');
        // letters.forEach(letter => {
        //   this.letterUrls[letter] = ''; //`${location.origin+location.pathname}/letters/${letter.toLocaleLowerCase()}.mp3`;
        // });
      ...
      // 预加载所有字母音频
      async preloadAllLetters(onProgress = null) {
        console.log('iiiiii')
        const letters = 'abcdefghijklmnopqrstuvwxyz'.split('');;
        let loadedCount = 0;
      ...
      // 播放字母音频（支持同时播放多个）
      playLetter(letter, volume = null, speed = 1.0) {
        // 如果字母音频已加载，直接播放
        if (this.letterAudios.has(letter)) {
      ...
      // 预加载所有在线音乐（句子或单词）
      async preloadAllOnlineAudios(texts, onProgress = null) {
        let loadedCount = 0;
      ...
      // 播放在线音乐（句子或单词）
      playOnlineAudio(text, volume = null, rate = 1.0, pitch = 1.0) {
        // 停止当前播放的在线音乐
      ...
      // 播放在线音频对象
      _playOnlineAudio(audio, name, volume, loop) {
        // 检查是否是 SpeechSynthesis 类型
        if (audio.type === 'speech') {
          speechSynthesis.speak(audio.utterance);
          return;
        }
      ...
      getCacheStatus() {
        return {
          lettersCached: this.letterAudios.size === 26,
          onlineAudios: Object.fromEntries(this.onlineAudios)
        };
      }
    }
